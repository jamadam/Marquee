...
現在のコンテキストを返します。これはローカライズされたC<$Marquee::CONTEXT>を参照します。
...
指定のパスのSSIテンプレートファイルを検索し、存在すればパスを返します。
