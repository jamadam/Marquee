...
L<Marquee>モジュールはアプリケーションのベースクラスであり、同梱のコマンドラインツール、
L<Mojolicious::Command::marquee>のバックエンドでもあります。
...
$instance->hooks->onのエイリアスです。指定のフックポイントにコールバックを追加します。下記のフックポイントが利用できます:
